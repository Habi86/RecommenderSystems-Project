# Implement a simple recommender (recommend_PB) that always predicts the overall most popular artists
# in the dataset, irrespective of a particular user profile and item content.
...
#                dict_rec_aidx = recommend_RB(np.setdiff1d(range(0, no_artists), u_aidx[train_aidx]), K_RB) # len(test_aidx))
                ...
                # Convert scores to votes (i.e., inverse ranks)
                votes = np.zeros(shape=(2, no_artists), dtype=np.int16)         # init rank matrix
                for m in range(0, scores.shape[0]):                             # for all methods to fuse
                    ...
                    for a in range(0, len(scores_sorted_idx)):
                        votes[m, aidx_nz[scores_sorted_idx[a]]] = a + 1
                # Sum ranks over different approaches
                ranks_fused = np.sum(votes, axis=0)
                # Sort and select top K_HR artists to recommend
